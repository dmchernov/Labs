...
namespace Izh_08_tasks
{
    ...
    public static class RefactoredBubbleSortClass
    {
        public delegate int SortFunc(int[] row);
        /// <summary>
        /// Метод сортирует строки матрица, используя указанный тип сортировки и критерий сравнения строк матрицы.
        /// </summary>
        /// <param name="matrix">Сортируемая матрица.</param>
        /// <param name="sortType">Тип сортировки enum.SortType.</param>
        /// <param name="criterionName">Критерий сортировки. Имя метода из класса Operations.</param>
        /// <returns>Отсортированная матрица.</returns>
        public static int[,] Sort(int[,] matrix, SortType sortType, string criterionName)
        {
            if (matrix == null)
            {
            ...
            for (int j = 1; j < matrix.GetLength(0); j++)
            {
                ...
                {
                    if (op.CompareOperation(criterionName, new object[] { sortFunc(GetRow(matrix, i)), sortFunc(GetRow(matrix, i + 1)) }))
                    {
